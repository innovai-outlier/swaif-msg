...
function Start-Service {
    ...
    if ([string]::IsNullOrWhiteSpace($Path)) {
        Write-Host "   X Error: Service path is empty." -ForegroundColor Red
        return $false
    }
    ...
    if (Test-Path $fullPath) {
        ...
        try {
            ...
            if ($LASTEXITCODE -eq 0) {
                Write-Host "   V $ServiceName started successfully!" -ForegroundColor Green
                Pop-Location
                return $true
            } 
            else {
                Write-Host "   X Error starting $ServiceName" -ForegroundColor Red
                Pop-Location
                return $false
            }
        } catch {
        ...
        Write-Host "   X Folder not found: $fullPath" -ForegroundColor Red
        return $false
    }
}
...
foreach ($result in $results) {
    if ($result["Success"]) {
        Write-Host "  V $($result["Service"])" -ForegroundColor Green
    } else {
        Write-Host "  X $($result["Service"])" -ForegroundColor Red
    }
}

...
# Add depths to path (needed before other imports)
sys.path.append(str(Path(__file__).parent.parent))

# Local imports after path setup
from depths.layers.l1_ingestion import L1Ingestion  # noqa: E402
from depths.layers.l2_grouper import L2Grouper  # noqa: E402
from depths.core.terminal_display import TerminalDisplay  # noqa: E402
    ...
    while True:
        try:
            ...
            if new_messages > 0:
                ...
                if conversations:
                    logger.info(
                        f"🔗 L2: Created/updated {len(conversations)} conversations"
                    )
...
def main():
    ...
    parser.add_argument(
        "--pipeline", action="store_true", help="Run full pipeline (L1 -> L2)"
    )
    parser.add_argument(
        "--process-l2", action="store_true", help="Process L2 grouping once"
    )
    parser.add_argument("--metrics", action="store_true", help="Show all metrics")
    parser.add_argument("--test", action="store_true", help="Test with json_test.json")

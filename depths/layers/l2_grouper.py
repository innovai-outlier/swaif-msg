...
class L2Grouper:
    ...
    def __init__(
        ...
        secretary_phone: str = "clinic_secretary",
    ):
        if database:
            self.db = database
        else:
    ...
    def generate_conversation_id(self, lead_phone: str, timestamp: str) -> str:
        ...
        with sqlite3.connect(self.db.db_path) as conn:
            ...
            row = conn.execute(
                """
                SELECT last_activity, conversation_id
                FROM lead_activity WHERE lead_phone = ?
                """,
                (clean_phone,),
            ).fetchone()
            ...
            conn.execute(
                """
                INSERT OR REPLACE INTO lead_activity
                (lead_phone, last_activity, conversation_id) VALUES (?, ?, ?)
                """,
    ...
    def process_pending_messages(self) -> List[Dict]:
        ...
        logger.info(
            f"✅ Grouped {len(messages)} messages into "
            f"{len(saved_conversations)} conversations"
        )
    ...
    def _group_into_conversations(self, messages: List) -> Dict:
        ...
        conversations: Dict[str, Any] = defaultdict(
            lambda: {
                "messages": [],
                "message_count": 0,
                "start_time": None,
        ...
        for msg in messages:
            ...
            participants = self.identify_participants(
                msg["sender_phone"], msg["receiver_phone"]
            )
            ...
            conv_id = self.generate_conversation_id(
                participants["lead_phone"], msg_time
            )
